...
export const askSaveProfile = async (dispatch: Dispatch, data: DeepPartial<DBProfile>) => {
    const res = await axios.post(`http://localhost:${process.env.PORT}/api/db/profile/save`, data);
...
export const askGetProfile = async (filter?: { [key in keyof DBProfile]: 0 | 1 | boolean }) => {
    return (await axios.post(`http://localhost:${process.env.PORT}/api/db/profile/get`, { filter })).data;
}
